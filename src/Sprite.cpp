...
void Sprite::move(GLFWwindow* window) {
    ...
    float speed = 3.0f;
    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS && this->currentCoord.y0 > 0 && !this->isCollision(0, -1)) {
    ...
    if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS && this->currentCoord.y1 < WINDOW_HEIGHT && !this->isCollision(0, 1)) {
        this->transformMatrix = glm::translate(this->transformMatrix, glm::vec3(0.f, speed * 1.f, 0.f));
        this->currentCoord.y0 += speed * 1.f;
        this->currentCoord.y1 += speed * 1.f;
    }

    if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS && this->currentCoord.x1 < WINDOW_WIDTH && !this->isCollision(1, 0)) {
    ...
    if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS && this->currentCoord.x0 > 0 && !this->isCollision(-1, 0)) {
        this->transformMatrix = glm::translate(this->transformMatrix, glm::vec3(-speed * 1.f, 0.f, 0.f));
        this->currentCoord.x0 -= speed * 1.f;
        this->currentCoord.x1 -= speed * 1.f;
    }
